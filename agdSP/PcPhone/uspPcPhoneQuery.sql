/****************************************************************
** Name: agdSp.uspPcPhoneQuery
** Desc: 電腦電話配對進階查詢
**
** Return values: 0 成功
** Return Recordset: 
** SeqNo            INT          - 流水號
** ComputerIP       VARCHAR(20)  - 電腦IP
** ComputerName     VARCHAR(50)  - 電腦名稱
** ExtCode          VARCHAR(20)  - 分機號碼
** Memo             NVARCHAR(200) - 備註
** CreateDT         DATETIME2(7) - 建立時間
** Creator          VARCHAR(20)  - 建立者
** UpdateDT         DATETIME2(7) - 更新時間
** Updator          VARCHAR(20)  - 更新者
** UpdatorName      NVARCHAR(20) - 更新者名稱
** Total            INT          - 資料總筆數
**
** Called by: 
**	AGD WebApi
**
** Parameters:
**	Input
** -----------
    @ComputerName    VARCHAR(50)  - 電腦名稱
	@ExtCode         VARCHAR(20)  - 分機號碼
	@Page 			  INT 			- 頁數
	@RowsPerPage 	  INT 			- 每頁筆數
	@SortColumn 	  NVARCHAR(30) 	- 排序欄位
	@SortOrder 		  VARCHAR(10) 	- 排序順序
**
**   Output
** -----------
	@ErrorMsg NVARCHAR(100) - 錯誤回傳訊息
** 
** Example:
** -----------
DECLARE @return_value INT
	,@ComputerName VARCHAR(50)
	,@ExtCode VARCHAR(20)
	,@Page INT = 1
	,@RowsPerPage INT = 20
	,@SortColumn NVARCHAR(30) = 'CreateDt'
	,@SortOrder VARCHAR(10) = 'ASC'
	,@ErrorMsg NVARCHAR(100)

	SET @ComputerName = ''
	SET @ExtCode = ''

EXEC @return_value = agdSp.uspPcPhoneQuery
	@ComputerName = @ComputerName
		,@ExtCode = @ExtCode
	,@Page = @Page
	,@RowsPerPage = @RowsPerPage
	,@SortColumn = @SortColumn
	,@SortOrder = @SortOrder
	,@ErrorMsg = @ErrorMsg OUTPUT

SELECT @return_value AS 'Return Value'
		,@ErrorMsg AS N'@ErrorMsg'
**
*****************************************************************
** Change History
*****************************************************************
** Date:            Author:         Description:
** ---------- ------- ------------------------------------
** 2022/04/11 14:11:34    Daniel Chou     first release
*****************************************************************/
CREATE PROCEDURE [agdSp].[uspPcPhoneQuery] (
	@ComputerName VARCHAR(50)
	,@ExtCode VARCHAR(20)
	,@Page INT = 1
	,@RowsPerPage INT = 20
	,@SortColumn NVARCHAR(30) = 'CreateDt'
	,@SortOrder VARCHAR(10) = 'ASC'
    ,@ErrorMsg NVARCHAR(100) = NULL OUTPUT
)
AS
SET NOCOUNT ON
SET @ErrorMsg = N''

BEGIN
	BEGIN TRY
		SELECT
            f.SeqNo
			,f.ComputerIP
			,f.ComputerName
			,f.ExtCode
			,f.Memo
			,f.CreateDT
			,f.Creator
			,f.UpdateDT
			,f.Updator
			,u.UserName AS UpdatorName
			,COUNT(f.SeqNo) OVER () AS Total
		FROM agdSet.tbPcPhone AS f
		JOIN agdSet.tbUser AS u ON u.UserId = f.Updator
		------- WHERE 查詢條件 -------
		WHERE  f.ComputerName LIKE CASE WHEN @ComputerName = '' THEN f.ComputerName ELSE '%' + @ComputerName + '%' END
				AND f.ExtCode LIKE CASE WHEN @ExtCode = '' THEN f.ExtCode ELSE '%' + @ExtCode + '%' END
				AND f.IsEnable = CASE WHEN @IsEnable = 'ALL' THEN f.IsEnable ELSE CASE WHEN @IsEnable = '1' THEN 1 ELSE 0 END END
		------- Sort 排序條件 -------
		ORDER BY 
				CASE WHEN @SortColumn = 'SeqNo' AND @SortOrder = 'ASC' THEN f.SeqNo END ASC,
				CASE WHEN @SortColumn = 'SeqNo' AND @SortOrder = 'DESC' THEN f.SeqNo END DESC,
				CASE WHEN @SortColumn = 'ComputerIP' AND @SortOrder = 'ASC' THEN f.ComputerIP END ASC,
				CASE WHEN @SortColumn = 'ComputerIP' AND @SortOrder = 'DESC' THEN f.ComputerIP END DESC,
				CASE WHEN @SortColumn = 'ComputerName' AND @SortOrder = 'ASC' THEN f.ComputerName END ASC,
				CASE WHEN @SortColumn = 'ComputerName' AND @SortOrder = 'DESC' THEN f.ComputerName END DESC,
				CASE WHEN @SortColumn = 'ExtCode' AND @SortOrder = 'ASC' THEN f.ExtCode END ASC,
				CASE WHEN @SortColumn = 'ExtCode' AND @SortOrder = 'DESC' THEN f.ExtCode END DESC,
				CASE WHEN @SortColumn = 'UpdateDT' AND @SortOrder = 'ASC' THEN f.UpdateDT END ASC,
				CASE WHEN @SortColumn = 'UpdateDT' AND @SortOrder = 'DESC' THEN f.UpdateDT END DESC
		------- Page 分頁條件 -------
		OFFSET @RowsPerPage * (@page - 1) ROWS

		FETCH NEXT @RowsPerPage ROWS ONLY
	END TRY

	BEGIN CATCH
		SELECT @ErrorMsg = LEFT(ERROR_MESSAGE(), 100)
	END CATCH
END

SET NOCOUNT OFF