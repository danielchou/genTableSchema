...
EXEC @return_value = [agdSp].[uspMessageUpdate]
    @SeqNo = @SeqNo
...
*****************************************************************/
CREATE PROCEDURE [agdSp].[uspMessageUpdate] (
	@SeqNo INT
	,@MessageSheetID VARCHAR(20)
	,@MessageTemplateID VARCHAR(20)
	,@MessageName NVARCHAR(50)
...
BEGIN
	BEGIN TRY
		UPDATE agdSet.tbMessage
		SET MessageSheetID = @MessageSheetID
			,MessageTemplateID = @MessageTemplateID
			,MessageName = @MessageName
			,DisplayOrder = @DisplayOrder
			,IsEnable = @IsEnable
