...
{
    public class MarketingUpdateValidator:AbstractValidator<MarketingUpdateRequest>
    {
        public MarketingUpdateValidator()
        {
			RuleFor(x => x.marketingID).NotEmpty().WithMessage("行銷方案代碼不能為空!!");
			RuleFor(x => x.marketingType).NotEmpty().WithMessage("行銷方案類別不能為空!!");
