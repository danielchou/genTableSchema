...
{
    public class ProtocolUpdateValidator:AbstractValidator<ProtocolUpdateRequest>
    {
        public ProtocolUpdateValidator()
        {
			RuleFor(x => x.protocol).NotEmpty().WithMessage("通路碼代碼不能為空!!");
			RuleFor(x => x.direction).NotEmpty().WithMessage("IN/OUT方向不能為空!!");
