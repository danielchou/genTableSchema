...
{
    public class MarketingActionCodeUpdateValidator:AbstractValidator<MarketingActionCodeUpdateRequest>
    {
        public MarketingActionCodeUpdateValidator()
        {
			RuleFor(x => x.actionCodeType).NotEmpty().WithMessage("客群方案類別不能為空!!");
			RuleFor(x => x.marketingID).NotEmpty().WithMessage("行銷方案代碼不能為空!!");
			RuleFor(x => x.actionCode).NotEmpty().WithMessage("行銷結果代碼不能為空!!");
