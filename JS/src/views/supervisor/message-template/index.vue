...
import messageTemplate from '@supervisor-store/message-template';
...
export default {
  ...
  setup() {
    ...
    !store.hasModule(`${moduleName}`) && store.registerModule(`${moduleName}`, messageTemplate);
    ...
    const filterItemOptions = [
      {
		 ...
		 placeholder: '請輸入訊息傳送範本代碼',
},
{
		 label: '訊息傳送範本名稱',
		 value: 'MessageTemplateName',
		 placeholder: '請輸入訊息傳送範本名稱',
},
{
		 label: '訊息傳送B08代碼',
		 value: 'MessageB08Code',
		 placeholder: '請輸入訊息傳送B08代碼',
},
    ];
