...
import marketingActionCode from '@supervisor-store/marketing-action-code';
...
export default {
  ...
  setup() {
    ...
    const moduleName = 'marketingActionCode';
    !store.hasModule(`${moduleName}`) && store.registerModule(`${moduleName}`, marketingActionCode);
    ...
    const filterItemOptions = [
      {
		 label: '客群方案類別',
		 value: 'ActionCodeType',
		 placeholder: '請輸入客群方案類別',
},
{
		 label: '行銷方案代碼',
		 value: 'MarketingID',
		 placeholder: '請輸入行銷方案代碼',
},
{
		 label: '行銷結果代碼',
		 value: 'ActionCode',
		 placeholder: '請輸入行銷結果代碼',
},
    ];
