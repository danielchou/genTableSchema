...
import notificationTemplate from '@supervisor-store/notification-template';
...
export default {
  ...
  setup() {
    ...
    const moduleName = 'notificationTemplate';
    !store.hasModule(`${moduleName}`) && store.registerModule(`${moduleName}`, notificationTemplate);
    ...
    const filterItemOptions = [
      {
		 ...
		 placeholder: '請輸入通知公告類別',
},
{
		 label: '通知公告代碼',
		 value: 'NotificationID',
		 placeholder: '請輸入通知公告代碼',
},
{
		 label: '通知公告名稱',
		 value: 'NotificationName',
		 placeholder: '請輸入通知公告名稱',
},
    ];
