...
import auxCode from '@supervisor-store/aux-code';
...
export default {
  ...
  setup() {
    ...
    const moduleName = 'auxCode';
    !store.hasModule(`${moduleName}`) && store.registerModule(`${moduleName}`, auxCode);
    ...
    const filterItemOptions = [
      {
		 label: '休息碼代碼',
		 value: 'AuxID',
		 placeholder: '請輸入休息碼代碼',
},
{
		 label: '休息碼名稱',
		 value: 'AuxName',
		 placeholder: '請輸入休息碼名稱',
},
    ];
