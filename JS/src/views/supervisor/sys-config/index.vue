...
import sysConfig from '@supervisor-store/sys-config';
...
export default {
  ...
  setup() {
    ...
    const moduleName = 'sysConfig';
    !store.hasModule(`${moduleName}`) && store.registerModule(`${moduleName}`, sysConfig);
    ...
    const filterItemOptions = [
      {
		 label: '系統參數類別',
		 value: 'SysConfigType',
		 placeholder: '請輸入系統參數類別',
},
{
		 label: '系統參數代碼',
		 value: 'SysConfigID',
		 placeholder: '請輸入系統參數代碼',
},
{
		 label: '系統參數名稱',
		 value: 'SysConfigName',
		 placeholder: '請輸入系統參數名稱',
},
    ];
