...
import protocol from '@supervisor-store/protocol';
...
export default {
  ...
  setup() {
    ...
    !store.hasModule(`${moduleName}`) && store.registerModule(`${moduleName}`, protocol);
    ...
    const filterItemOptions = [
      {
		 label: '通路碼代碼',
		 value: 'Protocol',
		 placeholder: '請輸入通路碼代碼',
},
{
		 label: '通路碼名稱',
		 value: 'ProtocolName',
		 placeholder: '請輸入通路碼名稱',
},
    ];
