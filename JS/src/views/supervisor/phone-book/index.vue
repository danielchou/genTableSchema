...
import phoneBook from '@supervisor-store/phone-book';
...
const columns = [
	...
	{
		 name: 'parentPhoneBookID',
		 label: '上層電話簿代碼',
		 align: 'left',
		 field: 'parentPhoneBookID',
		 sortable: true,
	},
	{
...
export default {
  ...
  setup() {
    ...
    const moduleName = 'phoneBook';
    !store.hasModule(`${moduleName}`) && store.registerModule(`${moduleName}`, phoneBook);
    ...
    const filterItemOptions = [
      {
		 label: '電話簿代碼',
		 value: 'PhoneBookID',
		 placeholder: '請輸入電話簿代碼',
},
{
		 label: '電話簿名稱',
		 value: 'PhoneBookName',
		 placeholder: '請輸入電話簿名稱',
},
{
		 label: '上層電話簿代碼',
		 value: 'ParentPhoneBookID',
		 placeholder: '請輸入上層電話簿代碼',
},
{
		 label: '電話號碼',
