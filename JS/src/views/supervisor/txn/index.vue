...
import txn from '@supervisor-store/txn';
...
export default {
  ...
  setup() {
    ...
    const moduleName = 'txn';
    !store.hasModule(`${moduleName}`) && store.registerModule(`${moduleName}`, txn);
    ...
    const filterItemOptions = [
      {
		 label: '交易執行類別',
		 value: 'TxnType',
		 placeholder: '請輸入交易執行類別',
},
{
		 label: '交易執行代碼',
		 value: 'TxnID',
		 placeholder: '請輸入交易執行代碼',
},
{
		 label: '交易執行名稱',
		 value: 'TxnName',
		 placeholder: '請輸入交易執行名稱',
},
    ];
